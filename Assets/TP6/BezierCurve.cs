...
{
    public Material matSelected, matNoSelected;
    public Transform[] controlPoints = new Transform[4]; // 4 points de contrôle
    private int selectedPointIndex = -1; // Point de contrôle sélectionné (-1 = aucun)
    [Range(2, 100)]
    ...
    private void HandleControlPointSelection()
    {
        ...
        foreach (var point in controlPoints)
        {
            ...
            {
                point.gameObject.GetComponent<MeshRenderer>().materials[0] = matSelected;
            }
            else
            {
                point.gameObject.GetComponent<MeshRenderer>().materials[0] = matNoSelected;
            }
            i++;
        }
    }
